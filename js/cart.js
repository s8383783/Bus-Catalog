...
function clearCart() {
  let tableSelect = document.querySelector('tbody')
  let removeRow = document.getElementsByClassName('cartLine');

  
  tableSelect.removeChild(removeRow);

  // tr.innerHTML = 'empty string';
}
...
function showCart(items) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    tr.setAttribute('class', 'cartLine');
  ...
  let clearCartButton = document.getElementById('cart-container');
  let cartButton = document.createElement('button')
  cartButton.textContent ='Clear Cart';
  clearCartButton.appendChild(cartButton);
